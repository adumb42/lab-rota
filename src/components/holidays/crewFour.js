...
class CrewFour extends React.Component {
    ...
    renderUnapprovedHolidays() {
        ...
        return this.props.holidays.map(holiday => {
            if (holiday.crewFourApproved === false) {
            ...
            } if (holiday.crewFourApproved === true) {
            ...
            if (holiday.crewFourEmail === false) {
            ...
            let updatedAtDate = moment(holiday.crewFourUpdatedAt).format('DD-MM-YYYY')
            while (
                holiday.crewFour === false &&
                ...
                return (
                    <tbody className="maintable" key={holiday.id}>
                        <tr>
                            ...
                            <td> <button className={emailButton} onClick={() => this.props.approvalToggle(holiday._id, { "crewFourEmail": !holiday.crewFourEmail })}>{emailMessage}</button></td>
                        </tr>
                    </tbody>
                )
            }
    ...
    render() {
        ...
        const booked = this.props.holidays.filter(holiday => holiday.crewFour === false && holiday.id > 364).length
        const swapsBooked = 227 - this.props.holidays.filter(holiday => holiday.crewFour === null).length
