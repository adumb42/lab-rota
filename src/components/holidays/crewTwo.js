...
class CrewTwo extends React.Component {
    ...
    renderUnapprovedHolidays() {
        ...
        return this.props.holidays.map(holiday => {
            if (holiday.crewTwoApproved === false) {
            ...
            } if (holiday.crewTwoApproved === true) {
            ...
            let updatedAtDate = moment(holiday.crewTwoUpdatedAt).format('DD-MM-YYYY')
            while (
                holiday.crewTwo === false &&
                ...
                return (
                    <tbody className="maintable" key={holiday.id}>
                        <tr>
                            ...
                            <td> <button className={emailButton} onClick={() => this.props.approvalToggle(holiday._id, { "crewTwoEmail": !holiday.crewTwoEmail })}>{emailMessage}</button></td>
                        </tr>
                    </tbody>
                )
            }
    ...
    render() {
        ...
        const booked = this.props.holidays.filter(holiday => holiday.crewTwo === false && holiday.id > 364).length
        const swapsBooked = 231 - this.props.holidays.filter(holiday => holiday.crewTwo === null).length
